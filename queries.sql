...
/*timeline*/
select "story" as type, c.idnum as author, c.chapter as chapter, c.tstamp as posted from Identity as a inner join Follows as b on (a.idnum = b.follower) inner join Story as c on (b.followed = c.idnum) where a.handle = "@snapoleon0" and c.tstamp between "2020-04-28 10:09:58" and "2020-04-28 10:10:00" and not exists((select 1 from Identity as xx inner join Block as qq on (xx.idnum = qq.blocked) where xx.handle = "@snapoleon0" and qq.idnum = c.idnum)) UNION select "reprint", y.idnum, y.chapter, y.tstamp from Identity as q inner join Follows as x on (q.idnum = x.follower) inner join Reprint as t on (x.followed = t.idnum) inner join Story as y on (t.sidnum = y.sidnum) where q.handle = "@snapoleon0" and y.tstamp between "2020-04-28 10:09:58" and "2020-04-28 10:10:00" and t.likeit is false and not exists((select 1 from Identity as x inner join Block as q on (x.idnum = q.blocked) where x.handle = "@snapoleon0" and q.idnum = y.idnum))order by posted desc;
